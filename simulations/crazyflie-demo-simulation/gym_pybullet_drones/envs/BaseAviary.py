...
class BaseAviary(gym.Env):
    ...
    def __init__(self,
        ...
        if self.GUI:
            ...
            if self.RECORD:
                ...
                self.FRAME_PER_SEC = 96
                self.CAPTURE_FREQ = int(self.SIM_FREQ/self.FRAME_PER_SEC)
                self.CAM_VIEW = p.computeViewMatrixFromYawPitchRoll(distance=0.8,
                                                                    yaw=0,
                                                                    pitch=30,
                                                                    roll=0,
                                                                    cameraTargetPosition=[-0.2, 0, 1.3],
                                                                    upAxisIndex=2,
                                                                    physicsClientId=self.CLIENT
                                                                    )
    ...
    def _dynamics(self,
        ...
        quat_v = 0.5*self.quaternion_multiply(np.array([0, ang_v[0], ang_v[1], ang_v[2]]), quat)
        #### Compute forces and torques ############################
        forces = np.array(rpm**2) * self.KF
        thrust = np.array([0, 0, np.sum(forces)])
        # thrust_world_frame = np.dot(rotation, thrust)
        conj_quat = np.array([quat[0], -quat[1], -quat[2], -quat[3]])
        thrust_tmp = self.quaternion_multiply(quat, np.array([0, thrust[0], thrust[1], thrust[2]]))
        thrust_world_frame = self.quaternion_multiply(thrust_tmp, conj_quat)
        force_world_frame = thrust_world_frame[1:4] - np.array([0, 0, self.GRAVITY])
        ...
        quat = quat + self.TIMESTEP * quat_v
        q0 = quat[0]
        quat[0:3] = quat[1:4]
        quat[3] = q0
    ...
    def _addObstacles(self):
        ...
        import os
        cwd = os.getcwd()
        p.setAdditionalSearchPath( cwd + "/bspline_static/urdf")
        ...
        #
        # textureId = p.loadTexture('cracked_0085.jpg')
        # p.changeVisualShape(multiBodyId, -1, textureUniqueId=textureId)

        # textureId = p.loadTexture("Sztaki.jpg")
