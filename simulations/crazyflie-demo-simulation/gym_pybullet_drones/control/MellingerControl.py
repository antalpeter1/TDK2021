...
class MellingerControl(BaseControl):
    """Mellinger controller class for Crazyflies.
    ...
    def __init__(self,
        ...
        if self.DRONE_MODEL != DroneModel.CF2X and self.DRONE_MODEL != DroneModel.CF2P:
            ...
            exit()
        self.k_r = np.diag([0.4, 0.4, 1.25])
        self.k_v = np.diag([0.2, 0.2, 0.4])
        self.k_R = np.diag([0.0095, 0.0095, 0.0126])
        self.k_w = np.diag([0.0027, 0.0027, 0.0025])

        self.kd_omega_rp = 0 # 2.72e-5
        self.inertia = np.diag([1.4e-5, 1.4e-5, 2.17e-5])
        # self.inertia = np.diag([0, 0, 0])
        self.mass = 0.028
        self.gravity = 9.81
    ...
    def computeControl(self,
        ...
        ###################
        p_target = np.zeros(3)
        target_ang_acc = np.zeros(3)

        if 0.9/control_timestep > self.control_counter > 0.1/control_timestep:
            ...
            # print(target_ang_vel[1])

            target_ang_acc = (target_ang_vel-self.prev_target_ang_vel) / control_timestep

            q2 = np.sqrt(1-q0**2)
            target_quat = np.array([0, q2, 0, q0])
            target_rotation = (Rotation.from_quat(target_quat)).as_matrix()
            # target_rotation = (Rotation.from_euler('xyz', np.array([0, self.pos_data[self.control_counter - 49, 2], 0]))).as_matrix()
            cur_rotation = np.array(p.getMatrixFromQuaternion(cur_quat)).reshape(3, 3)
            trace = np.trace(np.dot((target_rotation.transpose()), cur_rotation))
            # trace = -0.75
            rot_e = 1 / 2 / np.sqrt(1 + trace) * \
            ...
            ang_acc_e = (cur_ang_vel - self.prev_ang_vel) / control_timestep - target_ang_acc
            ang_acc_e[2] = 0
            target_torques = -self.k_R @ rot_e - self.k_w @ ang_vel_e + \
                             np.cross(cur_ang_vel, np.dot(self.inertia, cur_ang_vel)) + self.kd_omega_rp * ang_acc_e
        ...
        ##########################
            ...
            temp = -self.k_r@pos_e - self.k_v@vel_e + self.mass*self.gravity*np.array([0, 0, 1]) + self.mass*ddr_d
        ...
        # print(target_euler[1])

        cur_rpy = p.getEulerFromQuaternion(cur_quat)

        self.prev_ang_vel = cur_ang_vel
