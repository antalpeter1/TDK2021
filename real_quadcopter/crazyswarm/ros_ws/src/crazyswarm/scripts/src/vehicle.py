...
class Vehicle(Environment):
    ...
    def gurobi_path(self):
        ...
        for obs in self.obstacles:
            ...
            # obs.dx = 0.1
            # obs.dy = 0.1
            # model.addConstrs(( -x[i] + obs.corners[0][0] >=  obs.dx - R * c[0, i] for i in range(N - 1)  ))
            # model.addConstrs((  x[i] - obs.corners[1][0] >=  obs.dx - R * c[1, i] for i in range(N - 1)  ))
            # model.addConstrs(( -y[i] + obs.corners[0][1] >=  obs.dy - R * c[2, i] for i in range(N - 1)  ))
            # model.addConstrs((  y[i] - obs.corners[2][1] >=  obs.dy - R * c[3, i] for i in range(N - 1)  ))
        
            model.addConstrs(( -x[i] + obs.center[0] >=  obs.dx - R * c[0, i] for i in range(N - 1)  ))
            model.addConstrs((  x[i] - obs.center[0] >=  obs.dx - R * c[1, i] for i in range(N - 1)  ))
            model.addConstrs(( -y[i] + obs.center[1] >=  obs.dy - R * c[2, i] for i in range(N - 1)  ))
            model.addConstrs((  y[i] - obs.center[1] >=  obs.dy - R * c[3, i] for i in range(N - 1)  ))
        
            print(obs.corners[0][0])
            print(obs.corners[1][0])
            print(obs.corners[0][1])
            print(obs.corners[2][1])
        ...
        epsilon = 0.01
        for i in range(N):
            J += b[i] * i  + ax[i]**2 * epsilon + ay[i]**2 * epsilon
    ...
    def initialize_x(self):
        self.astar_path()
        print("Astar is done :), vehile: " + str(self.ID) + ", stage: " + str(self.stage))
        # self.gurobi_path()
    ...
    def plot_vehicle_trajectories(self, ax):
        ...
        for obstacle in self.obstacles:
            obstacle.plot_obstacle(ax)
            # corners = np.array(obstacle.corners)
